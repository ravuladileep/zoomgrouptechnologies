...
export class AddRoleComponent extends FormCanDeactivate implements OnInit {
  ...
  private addCheckboxesAssignFunction() {
    this.assignFunctionData.forEach((o, i) => {
      ...
      (this.addRoleSpecificForm.controls.assignFunction as FormArray).push(
        control
      );
    });
  }
  ...
  private addCheckboxesCommunication() {
    this.communicationData.forEach((o, i) => {
      const control = new FormControl(); // i===0 if first item set to true, else false
      (this.addRoleSpecificForm.controls.communication as FormArray).push(
        control
      );
    });
  }
  ...
  public submit(): void {
    ...
    this.roleService
      .addRole(this.addRoleSpecificForm.value)
      .pipe(untilDestroyed(this))
      .subscribe((res) => {
