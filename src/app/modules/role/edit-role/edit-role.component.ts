...
export class EditRoleComponent implements OnInit {
  ...
  private addCheckboxesAssignFunction() {
    this.assignFunctionData.forEach((o, i) => {
      ...
      (this.updateRoleSpecificForm.controls.assignFunction as FormArray).push(
        control
      );
    });
  }
  ...
  private addCheckboxesCommunication() {
    this.communicationData.forEach((o, i) => {
      const control = new FormControl(); // i===0 if first item set to true, else false
      (this.updateRoleSpecificForm.controls.communication as FormArray).push(
        control
      );
    });
  }
  ...
  public editRole(data): void {
    this.roleService
      ...
      .subscribe((res) => {
        ...
        const myassign = this.updateRoleSpecificForm.controls
          .assignFunction as FormArray;
        const myreports = this.updateRoleSpecificForm.controls
          .reports as FormArray;
        const mycommunication = this.updateRoleSpecificForm.controls
          .communication as FormArray;
        ...
        res.communication.forEach((x) => {
          mycommunication
            .at(this.communicationDataarr.indexOf(x))
            .patchValue(x);
        });
        //  end patching
      });
